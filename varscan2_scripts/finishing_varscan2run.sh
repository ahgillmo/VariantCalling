...
#SBATCH --job-name=finish_varscan_snv_loh_germline
#SBATCH --mem=22G
...
#Step 5 - Combine snp and indel vcfs from the same sample (similar to rbinding)
...
bcftools concat -Ov -a -R /home/ahgillmo/references/broad_hg38_wgs_callingregions.interval.bed -o $3".varscan.hc.vcf" $3".snp.Somatic.hc.vcf.gz" $3".indel.Somatic.hc.vcf.gz" &&

#Step 6 - Left align and split multiallelic variants -- output uncompressed vcf
bcftools norm -m-both -f /home/ahgillmo/references/grch38_2-1-0/refdata-GRCh38-2.1.0/fasta/genome.fa -o $3".varscan.leftaligned.hc.vcf" -Ov $3".varscan.hc.vcf"
...
bam-readcount -w 1 -q 1 -b 20 -l $3".varscan.leftaligned.hc.vcf.var" -f /home/ahgillmo/references/grch38_2-1-0/refdata-GRCh38-2.1.0/fasta/genome.fa $1 > $3".varscan.leftaligned.hc.vcf.var.readcount"
...
echo "Completed finishing Varscan2 with processing for bcftools union"
